...
public:
    typedef struct {
        ...
        float total_runtime;
        int thread_qty;
        float avg_thread_runtime;
        float thread_merge_runtime;
...
private:
    ...
    std::string GetSubPathNameByIndex(const std::string &root_path,
                                      const int id);
    std::vector<std::string> GetAllSubPathNames(
